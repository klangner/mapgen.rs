...
impl DrunkardsWalk {
    ...
    fn build(&self, rng: &mut StdRng, map: &Map) -> Map {
        ...
        new_map.set_tile(starting_position.x, starting_position.y, TileType::Unblocked(0));

        let total_tiles = new_map.width * new_map.height;
        let desired_floor_tiles = (self.floor_percent * total_tiles as f32) as usize;
        let mut floor_tile_count = new_map.tiles.iter().filter(|a| **a == TileType::Unblocked(0)).count();
        ...
        while floor_tile_count  < desired_floor_tiles {
            ...
            while drunk_life > 0 {
                new_map.set_tile(drunk_x, drunk_y, TileType::Blocked(0)); 
            ...
            digger_count += 1;
            floor_tile_count = new_map.tiles.iter().filter(|a| **a == TileType::Unblocked(0)).count();
        }
